...
class TWEC:
    ...
    def train_compass(self, compass_text, overwrite=False, filename="compass.model"):
        compass_exists = os.path.isfile(os.path.join(self.opath, filename))
        if compass_exists and overwrite is False:
            self.compass = Word2Vec.load(os.path.join(self.opath, filename))
            ...
            self.compass.save(os.path.join(self.opath, filename))

        self.gvocab = self.compass.wv.vocab

    def train_slice(self, slice_text, save=True, saveName=None):
        if self.compass == None:
        ...
        if save:
            if saveName:
                model.save(os.path.join(self.opath, saveName + ".model"))
            else:
